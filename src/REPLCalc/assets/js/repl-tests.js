export function createTests({
  ...
  function createTestSuite(){
    return [
      ...
      { name: "compound feet inches literal", expr: "6ft2in", expect: expectQty(6 + (2 / 12), "len") },
      { name: "compound feet inches literal assignment", steps: ["wall = 8ft4in", "wall"], expect: expectQty(8 + (4 / 12), "len") },
      { name: "area unit token with digit", expr: "12in2", expect: expectQty(12 * (1 / 144), "area") },
      ...
      { name: "tax", expr: "tax(100, 8)", expect: 108 },
      { name: "contingency", expr: "contingency(200, 5)", expect: 210 },
      { name: "overhead and profit", expr: "ohp(100, 10, 20)", expect: 132 },
      { name: "discount", expr: "discount(100, 15)", expect: 85 },
      { name: "retainage", expr: "retainage(100, 10)", expect: 90 },
      { name: "escalate", expr: "escalate(100, 3, 2)", expect: expectNear(106.09) },
      ...
      {
        name: "string helpers str and cat",
        steps: ["s = str(12 ft)", "c = cat(\"A\", 1, \"B\")", "if(s == \"12 ft\" && c == \"A1B\", 1, 0)"],
        expect: 1,
      },
      { name: "unit string ustr", expr: "ustr(3 ft)", expect: "3 ft" },
      {
        name: "seeded sample deterministic",
        steps: [
          "seed(123)",
          "a = sample(dist.uniform(0, 1))",
          "seed(123)",
          "b = sample(dist.uniform(0, 1))",
          "if(abs(a - b) < 1e-9, 1, 0)",
        ],
        expect: 1,
      },
      { name: "triangular dist cdf", expr: "cdf(dist.tri(0, 10, 5), 5)", expect: expectNear(0.5, 1e-9) },
      { name: "pvalue two-sided normal", expr: "pvalue(dist.normal(0, 1), 0)", expect: 1 },
      { name: "mean of range", expr: "mean(range(0, 10))", expect: 5 },
      {
        name: "argmax grid search",
        steps: ["r = argmax(\"x\", -2, 2, 1, \"-(x-1)^2\")", "field(r, \"x\")"],
        expect: 1,
      },
      {
        name: "dim and is_dim helpers",
        steps: ["d = dim(3 ft)", "field(d, \"len\") + is_dim(3 ft, \"len\")"],
        expect: 2,
      },
      { name: "unit mix umix", expr: "to_ft(umix(0 ft, 10 ft, 0.25))", expect: 2.5 },
      { name: "time conversions to_sec/to_min", expr: "to_sec(2 min) + to_min(120 sec)", expect: 122 },
      { name: "csi div", expr: "csi_div(\"06-11-16\")", expect: "06" },
      { name: "csi section", expr: "csi_section(\"06-11-16\")", expect: "06 11" },
      { name: "csi item", expr: "csi_item(\"06-11-16\")", expect: "06 11 16" },
      {
        name: "csi children list",
        expr: "csi_children(\"01 40 00\")",
        expect: "01 42 00,01 43 00,01 43 36,01 45 00",
      },
      {
        name: "graph gsum",
        steps: ["g = graph(\"g\")", "gnode(g, \"a\", 10)", "gnode(g, \"b\", 5)", "gsum(g)"],
        expect: 15,
      },
      {
        name: "rate name and is_rate",
        steps: ["r = rate(\"place\", 100 sfph, {crew: 2})", "if(is_rate(r) == 1 && rate_name(r) == \"place\", 1, 0)"],
        expect: 1,
      },
      {
        name: "matrix shape",
        steps: ["m = mat(\"1,2;3,4;5,6\")", "shape = mat_shape(m)", "field(shape, \"rows\") * 10 + field(shape, \"cols\")"],
        expect: 32,
      },
      {
        ...
        steps: [
          ...
          "p4 = pt(0 ft, 5 ft)",
          "shape = poly(p1, p2, p3, p4)",
          "to_ft(poly_perim(shape))",
        ],
        expect: 30,
      },
      { name: "flooring sf with default waste", expr: "to_sf(flooring_sf(100 sf))", expect: 110 },
      { name: "base trim lf with default waste", expr: "to_ft(base_trim_lf(100 ft))", expect: 110 },
      { name: "stud_count uses oc_count", expr: "stud_count(10 ft, 16 in)", expect: expectQty(8, "count") },
      { name: "light_fixture returns spec", expr: "field(light_fixture(\"TROFFER_2X4\"), \"watts\")", expect: 40 },
      { name: "light_fixtures applies waste", expr: "light_fixtures(10, 10)", expect: expectQty(11, "count") },
      { name: "stair treads waste", expr: "stair_treads(10, 10)", expect: expectQty(11, "count") },
      { name: "stair risers waste", expr: "stair_risers(10, 10)", expect: expectQty(11, "count") },
      {
        name: "light takeoff totals",
        steps: ["f = light_fixture(\"DOWNLIGHT_4\")", "t = light_takeoff(f, 10, 0)", "field(t, \"watts\")"],
        expect: 120,
      },
      { name: "duct gauge thickness", expr: "to_in(duct_gauge_thk(24))", expect: expectNear(0.0236, 1e-6) },
      { name: "duct lf waste", expr: "to_ft(duct_lf(100 ft, 10))", expect: 110 },
      { name: "duct elbows waste", expr: "duct_elbows(10, 10)", expect: expectQty(11, "count") },
      { name: "duct diffusers waste", expr: "duct_diffusers(10, 10)", expect: expectQty(11, "count") },
      {
        name: "duct takeoff + weight",
        steps: [
          "spec = duct_spec(\"rect\", 12 in, 8 in, 0.02 in, 100 pcf)",
          "take = duct_takeoff(spec, 10 ft, 0)",
          "lf = to_ft(field(take, \"lf\"))",
          "area = to_sf(field(take, \"area\"))",
          "wt = to_lb(field(take, \"weight\"))",
          "if(abs(lf - 10) < 1e-6 && abs(area - 33.3333333333) < 1e-6 && abs(wt - 5.5555555556) < 1e-6, 1, 0)",
        ],
        expect: 1,
      },
      {
        name: "duct weight helper",
        steps: ["spec = duct_spec(\"rect\", 12 in, 8 in, 0.02 in, 100 pcf)", "to_lb(duct_weight(spec, 10 ft, 0))"],
        expect: expectNear(5.5555555556, 1e-6),
      },
      {
        name: "dungeon generator map size",
        steps: ["d = dungeon(42, 10, 9, 2)", "mw(d) * mh(d)"],
        expect: 90,
      },
      { name: "to_csv qty", expr: "to_csv(12 ft)", expect: "value,kind\n12,len" },
      {
        name: "from_csv key/value",
        steps: ["csv = \"key,value\\nfoo,3\\nbar,baz\"", "field(from_csv(csv), \"bar\")"],
        expect: "baz",
      },
      {
        name: "finish ids not empty",
        steps: ["ids = finishIds()", "if(ids != \"\", 1, 0)"],
        expect: 1,
      },
      {
        name: "finish selection helpers",
        steps: [
          "w = wallFinish(\"paint white\")",
          "f = floorFinish(\"tile 12x12\")",
          "c = ceilingFinish(\"act 2x4\")",
          "t = trimFinish(\"base\", \"rubber base\")",
          "ws = wainscot(4 ft, \"wainscot\")",
          "if(w == \"PAINT_FLAT_WHITE\" && f == \"TILE_CERAMIC_GRAY_12X12\" && c == \"ACT_2x4\" && t == \"RUBBER_BASE_BLACK\" && ws == \"WAINSCOT_BEADBOARD_WHITE\", 1, 0)",
        ],
        expect: 1,
      },
      { name: "finish info lookup", expr: "field(finishInfo(\"PAINT_FLAT_WHITE\"), \"group\")", expect: "wall" },
      {
        name: "material vol_from_wt",
        steps: [
          "conc = material(\"concrete\", {density: 150 pcf, unit_cost: 125 $/cy})",
          "to_cf(vol_from_wt(300 lb, conc))",
        ],
        expect: 2,
      },
      {
        name: "trench cy volume",
        expr: "to_cy(trench_cy(100 ft, 2 ft, 1 ft, 0.5))",
        expect: expectNear(400 / 27, 1e-6),
      },
      {
        name: "not builtin",
        expr: "not(0) + not(1)",
