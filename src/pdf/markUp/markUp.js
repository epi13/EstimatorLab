 ...
 */
 ...
 class PageView {
   ...
   refreshAnnotations(){
     ...
     // Selection box etc — handled per-object
   }
   drawObject(obj){
     ...
     switch(obj.type){
       ...
       case 'note':{
         ...
         g.appendChild(note);
         const t=document.createElementNS('http://www.w3.org/2000/svg','text'); t.textContent='🗒️'; t.setAttribute('x','2'); t.setAttribute('y','14'); t.setAttribute('font-size','14'); g.appendChild(t);
   ...
   bboxOf(obj){
     // Approx bbox based on data bounds; transform not applied — selection rect uses local space for simplicity
     switch(obj.type){
 ...
 function refreshLayers(){
   ...
   for(const id of ids){ 
     ...
     row.style.gap='8px';
     const name=document.createElement('div'); name.textContent=`${obj.type} • ${id}`;
 ...
 function insertStamp(){
   ...
   const id=Util.uid();
   const now=new Date();
   const label=`REVISED — ${now.toLocaleDateString()} ${now.toLocaleTimeString()}`;
 ...
 function addCommentFor(obj){
   ...
   const input=document.createElement('textarea'); input.placeholder='Reply…'; input.rows=2; input.style.width='100%';
